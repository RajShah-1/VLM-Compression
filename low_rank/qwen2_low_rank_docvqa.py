...
def evaluate_model(model, dataset=None, benchmark_cls=VQA_v2):
    ...
    print(f"Size: {model.get_model_size()}, Avg Time: {model.get_average_processing_time()} Duration: {duration}s")
    now = datetime.now()
...
def main(retained_var):
    ...
    # custom_model = CustomQwen2VL.from_low_rank_path(metadata, weights, f'{dir_name}_vqa2')

    from benchmark.docvqa import DocVQA
    evaluate_model(custom_model, eval_dataset, DocVQA)
