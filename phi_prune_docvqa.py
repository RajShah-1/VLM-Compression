...
def evaluate_model(model, processor, eval_dataset, output_dir):
    ...
    # Calculate BERTScore
    references = [random.choice(ans) for ans in answers]  # Randomly select one reference answer per question
    bert_score_results = bertscore.compute(predictions=generated_texts, references=references, lang="en")

    # Save evaluation results
    # Save evaluation results
    evaluation_path = os.path.join(output_dir, "evaluation_results_docvqa.json")
    with open(evaluation_path, 'w') as f:
        results = {
            "answers": answers,
            "generated_texts": generated_texts,
            "bertscore_precision": bert_score_results["precision"],
            "bertscore_recall": bert_score_results["recall"],
            "bertscore_f1": bert_score_results["f1"],
        }
        json.dump(results, f, indent=4)
    end_time = time.time()  # End timer for evaluation
...
def main():
    ...
    trainer.train()
    fine_tuning_end = time.time()
    fine_tuning_time = fine_tuning_end - fine_tuning_start
    print(f"\nFine-tuning completed in {fine_tuning_time:.2f} seconds.\n")
