...
class Flickr30k(Benchmark):
    def __init__(self, model : Model):
        ...
        dataset = load_dataset("nlphuji/flickr30k", split="test", cache_dir=self.cache_dir)
    ...
    def evaluate(self):
        ...
        for i in tqdm(range(0, min(len(self.dataset), NUM_EXAMPLES), EVAL_BATCH_SIZE)):
            examples = self.dataset[i: i + EVAL_BATCH_SIZE]
            self.answers_unique.extend(examples["caption"])
            images = [[im] for im in examples["image"]]
            queries = [{"en": "What is happening in the image"} for im in examples["image"]]
            ...
            print("Expected is ", examples["caption"], flush=True)
